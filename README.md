...
Once the final random number has been determined a MOD is performed in order to generate a number within the possible range given the specified length of the secret random numbers. The winning tickets are those which tie for closest (may be over or under).
